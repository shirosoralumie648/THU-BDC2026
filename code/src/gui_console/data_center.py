...
def _render_job_logs(job_key: str, title: str) -> None:
    job = get_job(job_key)
    ...
    log_text = read_text_tail(job['log_path'], max_lines=160)
    st.text_area(f'{title} 日志 (tail)', value=log_text, height=180, key=f'{job_key}_tail')
...
def render_data_center(config_override_path: Optional[str]) -> None:
    st.subheader('数据中心 (Data Management)')
    effective_config = load_effective_config(config_override_path)

    fetch_col, split_col = st.columns(2)
    ...
    with fetch_col:
        ...
        output_path = st.text_input(
            '输出文件路径',
            value=resolve_dataset_path(effective_config, 'stock_data.csv'),
            key='fetch_output_path',
        )
        ...
        adjustflag_label = st.selectbox(
            '复权方式',
            options=['后复权(1)', '前复权(2)', '不复权(3)'],
            index=0,
            key='fetch_adjustflag',
        )
        ...
        with st.expander('高级抓取参数', expanded=False):
            ...
            request_interval = st.number_input(
                ...
                value=0.05,
                step=0.01,
                key='fetch_request_interval',
            )
            limit_stocks = st.number_input(
                '仅抓取前 N 只（0=全量）',
                min_value=0,
                max_value=300,
                value=0,
                step=1,
                key='fetch_limit_stocks',
            )
        ...
        run_fetch = st.button('开始抓取', key='btn_run_fetch')
        stop_fetch = st.button('停止抓取', key='btn_stop_fetch')
        ...
        if run_fetch:
            cmd = python_cmd(
                ...
                [
                    ...
                    str(float(request_interval)),
                    '--limit-stocks',
                    str(int(limit_stocks)),
                ],
            )
            if rebuild:
                cmd.append('--rebuild')
        ...
        _render_job_logs('fetch_data', '数据抓取')
    ...
    with split_col:
        ...
        run_split = st.button('执行切分', key='btn_run_split')
        stop_split = st.button('停止切分', key='btn_stop_split')
        ...
        if run_split and train_start is not None:
            cmd = python_cmd(
                ...
                [
                    ...
                    manifest_path,
                    '--train-start',
                    str(pd.to_datetime(train_start).date()),
                    '--train-end',
                    str(pd.to_datetime(train_end).date()),
                    '--test-start',
                    str(pd.to_datetime(test_start).date()),
                    '--test-end',
                    str(pd.to_datetime(test_end).date()),
                ],
            )
            try:
        ...
        _render_job_logs('split_data', '数据切分')
    ...
    with preview_col2:
        custom_path = st.text_input('自定义路径', value=resolve_dataset_path(effective_config, 'stock_data.csv'), key='preview_custom_path')
